...
trait_mean_male = [0.5, -0.5]
trait_mean_female = [-0.5, 0.5]
trait_std = 0.25
...
def reproduction(selected_individuals):
    ...
    # Select males based on assortative mating
    for _ in selected_individuals:
        min_trait_difference = float('inf')
        chosen_male = None
        chosen_female = np.random.choice(female_individuals)

        # Iterate through all males to find the one with the smallest trait difference
        for male in male_individuals:
            trait_difference = abs(chosen_female.genotype - male.genotype)
            if trait_difference < min_trait_difference:
                min_trait_difference = trait_difference
                chosen_male = male
...
locus= np.full((total_Loci, 2), (-1, 1), dtype=np.float64)
locus[0]= (1,1)
population = population + [Individual(sex=np.random.choice([Sex.MALE, Sex.FEMALE]), locus = locus) for _ in range(initial_population_size//2)]
global_data = {}
